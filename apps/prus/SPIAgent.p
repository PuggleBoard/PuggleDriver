...
ADC_LOOP:
  // ADC write bit
  QBBS ADC_MOSI_HIGH, SPI_RX, ADC_WRITE_COUNT
  ...
  // ADC read bit
  QBBS ADC_MISO_HIGH, SPI_RX.b0.t0

  CLR SPI_RX, ADC_COUNT
  JMP ADC_MISO_DONE

  ADC_MISO_HIGH:
    SET SPI_RX, ADC_COUNT

  ADC_MISO_DONE:
    // Keep running?
    SET SPI_SCLK
    SUB ADC_COUNT, ADC_COUNT, 1
    SUB ADC_WRITE_COUNT, ADC_WRITE_COUNT, 1
    QBNE ADC_LOOP, ADC_COUNT, 0

  // ADC write last bit
  delayOne
  QBBS ADC_FINAL_MOSI_HIGH, SPI_RX, ADC_WRITE_COUNT
  ...
  // ADC read last bit
  QBBS ADC_FINAL_MISO_HIGH, SPI_RX.b0.t0
...
DAC_LOOP: 
  // Configure MOSI
  QBBS DAC_MOSI_HIGH, SPI_TX, DAC_COUNT
  ...
  // Configure MOSI
  QBBS DAC_MOSI_HIGH_FINAL, SPI_TX, DAC_COUNT
